 ...
 *   - 'group' | 'gid'  Set process user
 *   - 'user' | 'uid'  Set process group
 *
 * Events:
 *
...
var Master = module.exports = function Master(server) {
  ...
  this.server = server;
  this.plugins = [];
  this.options = { 'working directory': '/' };
  this.children = [];
...
Master.prototype.start = function(){
  ...
  // signal handlers
  process.on('SIGINT', self.close.bind(self));
  process.on('SIGTERM', self.close.bind(self));
 ...
 * when all workers have shutdown.
 *
 * @api public
 */
...
Master.prototype.workerKilled = function(worker){
  ...
  switch (this.state) {
    case 'close':
      --this.pendingDeaths || this.destroy();
      break;
    default:
