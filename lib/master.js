...
/**
 * Return the path of the unix domain socket client used for IPC.
 *
 * @return {String}
 * @api public
 */

Master.prototype.__defineGetter__('clientSocketPath', function(){
  var pid = process.env.CLUSTER_MASTER_PID || this.pid;
  return this.options['socket path'] + '/cluster.' + pid + '.client.sock';
});
...
Master.prototype.startWorker = function(){
  var worker = this.worker = new Worker(this);
...
Master.prototype._destroy = function(){
  ...
  try { unlink(this.clientSocketPath); } catch (err) {}
