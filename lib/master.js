...
/**
 * Gracefully restart all workers, by sending __SIGQUIT__
 * to them, enabling master to re-spawn them.
 *
 * @api public
 */
...
Master.prototype.removeWorker = function(id){
  clearTimeout(this.children[id].timeout);
...
Master.prototype.kill = function(sig){
  ...
  this.children.forEach(function(worker){
    ...
    if ('SIGKILL' == sig) return;
    worker.timeout = setTimeout(function(){
      self.emit('worker timeout', worker, sig);
