...
module.exports = function(dir, level){
  return function(master){
    ...
    master.on('worker waiting', function(worker, connections){
      master.log.info('worker ' + worker.id + ' waiting on ' + connections + ' connections');
    });
    ...
    master.on('restart', function(){
      master.log.info('restart requsted');
    });
    ...
    master.on('worker', function(worker){
      ...
      if (proc.stdout.pipe) {
        ...
        proc.stdout.on('data', function(chunk){
          worker.accessLog.write(chunk);
        });
        proc.stderr.on('data', function(chunk){
          worker.errorLog.write(chunk);
        });
      }
    });
