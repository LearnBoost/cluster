 ...
 *   - `maxArgs` If CLI args exceed maxArgs than do NOT use watch files, this allows for --status etc. , defaulting to `2` ('node' & 'app.js')
 *
 * Examples:
 *
 *     cluster(server)
...
exports = module.exports = function(files, options){
  ...
  var sig = options.sig || 'SIGTERM'
	...
	, maxArgs = options.maxArgs || 2;

  return function(master){
    var restarting;
    
	if (process.argv.length > maxArgs) return;
