...
exports.monitor = function(master, sock){
  var workers = master.children.length
    , n = 0;

  // TODO: Worker#id
  master.broadcast('monitor');
  // TODO: refactor invokeCommand
  //       both master/worker support
  master.monitor = master.monitor || function(){
    // first
    if (0 == n++) {
      sock.write('\n');
      sock.head('Monitoring');
      sock.row('connecting', workers + ' workers');
      sock.write('  ');
    }
    sock.write('.');
    if (n == workers) {
