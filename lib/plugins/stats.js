    ...
    if (master.isWorker) {
      ...
      if (options.connections) {
        server.on('connection', function(sock){
          master.call('stats', 'connection');
          sock.on('close', function(){
            master.call('stats', 'disconnection');
          });
        });
      }
      ...
      if (options.requests) {
        server.on('request', function(req, res){
          ...
          master.call('stats', 'request', data);
        });
      }
    // master stats
    } else {
      ...
      // worker stats
      master.stats = function(worker, type, data){
...
repl.define('stats', function(master, sock){
  ...
  master.children.forEach(function(worker){
    ...
    // requests
    stats += ' ' + worker.stats.requestsTotal;
